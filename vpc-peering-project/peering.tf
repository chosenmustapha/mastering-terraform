...
# West-Central VPC Peering Connection
resource "aws_vpc_peering_connection" "west_central_peering" {
  provider      = aws.West_region
  peer_owner_id = data.aws_caller_identity.current.account_id
  ...
  tags = {
    Name = "west-central-peering"
  }
}

resource "aws_vpc_peering_connection_accepter" "west_central_peering_accepter" {
  provider                  = aws.Canada_region
  vpc_peering_connection_id = aws_vpc_peering_connection.west_central_peering.id
  auto_accept               = true

  depends_on = [aws_vpc_peering_connection.west_central_peering]

  tags = {
    Name = "west-central-peering-accepter"
  }
}